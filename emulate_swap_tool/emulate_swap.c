...
static unsigned long emulate_swap_mmap(Context *ctx, const This *this, const CallMmap *call) {
		unsigned long ret;
		RetUL *_ret = call->ret;
		...
		if (ret >= -4095UL && call->flags & MAP_ANONYMOUS) {
			...
			ret = (unsigned long)sys_mmap((void *)call->addr, call->len, call->prot, flags, fd, call->off);
			sys_close(fd);
		}
