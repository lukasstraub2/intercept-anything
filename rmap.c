...
static RMapEntry *_rmap_search_binary(RMap *this, const uint32_t id, int u, int o) {
	if (u > o) {
		return NULL;
	}
...
static RMapEntry *rmap_search_linear(RMap *this, const uint32_t id) {
	...
	for (int i = 0; i < size; i++) {
		...
		uint32_t current_tid = __atomic_load_n(&current_entry->id, __ATOMIC_RELAXED);
...
void _rmap_free(RMap *this, RMapEntry *entry) {
	...
	if (idx == actual_size -1) {
		__atomic_compare_exchange_n(&this->size, &expected, actual_size -1,
